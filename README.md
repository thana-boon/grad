...
- **ไม่**ดึงกลับเข้าระบบอัตโนมัติหลัง**เพิ่ง**ถูกเตะออกเพราะหมดเวลา (idle/token หมดอายุ) —
  ต้องกรอกเอง ไม่งั้น idle timeout จะไม่มีความหมาย และผู้ใช้จะไม่ทันเห็นว่าโดนเตะเพราะอะไร
  ...
  · ข้อยกเว้น: หลุดเพราะ **ตัวตนฝั่ง SchoolOS เปลี่ยน** (ข้อถัดไป) ไม่ต้องกรอกเอง —
...
- **กด "ออกจากระบบ" = ออกจาก SchoolOS ทั้งแพลตฟอร์มด้วย** จงใจให้เป็นแบบนี้ ถ้าล้างแค่ฝั่ง
  ...
  บนเครื่องส่วนกลาง
- **session จบเมื่อไร (กดออกเอง / หมดเวลา / token หมดอายุ) ผู้ใช้ถูกส่งไป `SCHOOLOS_PORTAL_URL`**
  ไม่ค้างอยู่ที่หน้า login ของ GradTrack · ตอนกดออกจากระบบใช้การ navigate ไป
  `/users/api/auth/logout?next=` ครั้งเดียว (เชื่อถือได้กว่ายิง POST ทิ้งไว้แล้วรีบเปลี่ยนหน้า
  ซึ่งเบราว์เซอร์ยกเลิกกลางคันได้)
  > ⚠️ ส่งออกเฉพาะ **จังหวะที่ session เพิ่งจบ** เท่านั้น ห้ามให้หน้า login เด้งออกเองตอน
  > render ไม่งั้นจะวนไม่รู้จบ portal → GradTrack → portal และ **บัญชี local จะไม่มีทาง
  > เข้าถึงฟอร์มได้เลย** ซึ่งเป็นทางเข้าสำรองตอน SchoolOS ล่ม
