...
- **กด "ออกจากระบบ" = ออกจาก SchoolOS ทั้งแพลตฟอร์มด้วย** จงใจให้เป็นแบบนี้ ถ้าล้างแค่ฝั่ง
  ...
  บนเครื่องส่วนกลาง
- **session จบเองยังไง (หมดเวลา / token หมดอายุ / 401 / SchoolOS ออกไปแล้ว) ก็ไปจบที่
  หน้า login ของ GradTrack เสมอ** พร้อมข้อความบอกเหตุผล — ไม่เด้งออกไป portal
  ผู้ใช้ที่หมดเวลาแล้วไปโผล่หน้า login ของ SchoolOS จะไม่รู้ว่าเกิดอะไรขึ้น และต้องเดินกลับ
  เข้ามาที่ `/grad` เองอีกรอบ · ที่หน้า login ถ้ายังล็อกอิน SchoolOS อยู่ silent SSO
  ก็พาเข้าใหม่ให้เองอยู่แล้ว (ทางเดียวกับที่ `SessionGuard` ใช้ — `bounceToLogin()`
  ใน `utils/session.js` โหลดหน้าใหม่ทั้งหน้าเสมอ ห้ามใช้ router เพราะหน้าที่ค้างอยู่
  ถูก render ไว้ให้คนเก่า)
- **`SCHOOLOS_PORTAL_URL` ใช้กับปุ่ม "ออกจากระบบ" อย่างเดียว** — กดออกเอง = ตั้งใจออกจาก
  แพลตฟอร์มด้วย จึง navigate ไป `/users/api/auth/logout?next=<portal>` ครั้งเดียว
  (เชื่อถือได้กว่ายิง POST ทิ้งไว้แล้วรีบเปลี่ยนหน้า ซึ่งเบราว์เซอร์ยกเลิกกลางคันได้)
  > ⚠️ ห้ามให้หน้า login เด้งออกไป portal เองตอน render ไม่ว่ากรณีใด ไม่งั้นจะวนไม่รู้จบ
  > portal → GradTrack → portal และ **บัญชี local จะไม่มีทางเข้าถึงฟอร์มได้เลย**
  > ซึ่งเป็นทางเข้าสำรองตอน SchoolOS ล่ม
